...
@torch.no_grad()
def speculative_sampling_v5(prefix : torch.Tensor, approx_models: List[torch.nn.Module], target_model : torch.nn.Module, learner : torch.nn.Module, 
                         max_len : int , gamma : int = 4,
                         temperature : float = 1, top_k : int = 0, top_p : float = 0, random_seed : int = None) -> torch.Tensor:
    """
    DeepMind version Speculative Sampling.
    ...
    while prefix.shape[1] < T:
        ...
        with autocast():
            ...
            # idx = learner.sample_drafter(learner_logits)
            dists = []
            p_logits = target_model.model(prefix).logits.to('cpu')
            for m in approx_models:
                diff = norm_logits(m.model(prefix).logits[:,-1,:].to('cpu'), temperature, top_k, top_p) - norm_logits(p_logits[:,-1,:], temperature, top_k, top_p)
                diff = torch.abs(diff)
                diff = torch.sum(diff)
                dists.append(diff.item())
            idx = np.argmin(dists)
            # idx = 2
            chosen_model = approx_models[idx]
